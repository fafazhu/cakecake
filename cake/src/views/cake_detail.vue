    ...
    <div class="container">
        ...
        <div class="content_box">
            ...
            <div class="right_content">
                <div>
                    <p><img :src="require('../assets/product'+image)" alt="">¥<span> {{product.price}}.00</span>/{{product.size}}</p>
                </div>
                <ul>
                    <li><i></i>约{{spec[1].items[sizei].size}}cmx{{spec[1].items[sizei].size}}cm</li>
                    <li><i></i>适合{{spec[1].items[sizei].renshu}}人分享</li>
                    <li><i></i>含{{spec[1].items[sizei].canju}}套餐具（蜡烛需单独订购）</li>
                    <li><i></i>最早明天 10:00配送</li>
                </ul>
                <div v-for="(item,i) of spec" :key="i" class="space">
                    <span>{{item.name}}</span>
                    <a href="javascript:;" v-for="(specs,index) of item.items" :key="index"  :class="{active:sel[0]==specs.name || sel[1]==specs.name}" @click="select(i,specs.name)">
                        <i></i>
                        {{specs.name}}
                    </a>
                    <!-- <a href="javascript:;">
...
export default {
    ...
    data() {
        return {
            image:"/0/3.jpg",
            imageList:["/0/3.jpg","/0/2.jpg","/0/1.jpg","/0/4.jpg"],
            ...
            spec:[
                ...
                {name:"商品规格",
                items:[{sid:1,name:"454g(1.0磅)",size:"13",renshu:"3-4",canju:5},{sid:2,name:"908g(2.0磅)",size:"17",renshu:"7-8",canju:10}]}
            ],
            product:[],
            sel:[],
            sizei:"1",
    ...
    methods: {
        ...
        select(i,index){
            this.sel[i]=index;
            getPidBySpec(this.sel[0],this.sel[1])
            .then(data=>{
                this.$router.replace("/cakeDetail/"+data[0].pid);
                this.load();
            })
            // for(var i=0;i<this.product.length;i++){
        ...
        load(){
            getDetailsByPid(this.$route.params.pid).then(data=>{
                this.product=data[0];
                this.image=this.imageList[0]="/" + data[0].img;
                this.sel.push(this.product.tuan,this.product.size);
                if(this.product.size=="908g(2.0磅)"){
                    this.sizei=1;
                }else{
                    this.sizei=0;
                }
